...
public class CartesianProduct {
    ...
    public static CartesianProductOfTwoRanges ofTwoRanges(int from1, int to1, int from2, int to2) {
        return new CartesianProductOfTwoRanges(validateAndGetRange(from1, to1), validateAndGetRange(from2, to2));
    }

    private static Iterable<Integer> validateAndGetRange(int from, int to) {
        if (from > to) {
    ...
    public static class CartesianProductOfTwoRanges {
        private final Iterable<Integer> range1;
        private final Iterable<Integer> range2;

        private CartesianProductOfTwoRanges(Iterable<Integer> range1, Iterable<Integer> range2) {
            this.range1 = range1;
            this.range2 = range2;
        }
