 ...
 */
public class ValuedStatusSupportImpl<K, V> implements FeatureSupport<Update> {
    private final Map<K, StatusHandler<K>> handlers;
    private final Map<K, ValuedStatusHandler<K, V>> valuedHandlers;
    private final TriFunction<Update, K, FeedbackExecutor, Boolean> unknownKeyHandler;
    ...
    public boolean handle(Update update, FeedbackExecutor feedbackExecutor) {
        ...
        if (status.value().isPresent()) {
            return Opt.ofNullable(valuedHandlers.get(key))
                    ...
                    .orElseGet(() -> unknownKeyHandler.apply(update, key, feedbackExecutor));
        }
