...
public class ClassesToTxtWithoutToDo {
    public static void main(String[] args) throws Exception {
        ...
        Files.write(Paths.get(".txt"), Files.readAllLines(Paths.get("list"))
                .stream()
    ...
    static List<String> sub(List<String> list) {
        int i = 0;
        for (String s : list)
            if (s.startsWith("public") || s.startsWith("@")) break;
            else i++;
    ...
    static Map<String, Path> collectJavaFiles() throws IOException {
        ...
        Files.walkFileTree(Paths.get("src/main/java/ru/prohor/universe/scarif"), new SimpleFileVisitor<>() {
            @Override
