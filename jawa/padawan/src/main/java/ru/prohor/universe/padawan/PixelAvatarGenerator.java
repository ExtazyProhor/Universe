...
public class PixelAvatarGenerator {
    ...
    private static final Color[] patternColors = {
            new Color(13, 13, 74),
            new Color(103, 0, 0),
            new Color(0, 64, 0),
            new Color(97, 74, 0),
    ...
    private static BufferedImage generate(Color bg, Color dice, Color dots, int value) {
        ...
        for (int[] dotsPosition : dicePatterns[value]) {
            fillRect(
                    image,
                    dots,
    ...
    private static void fillRect(BufferedImage image, Color color, int x, int y, int width, int height) {
    ...
    private static final int[][][] dicePatterns = {
            {{1, 1}}, // 1
            {{0, 0}, {2, 2}}, // 2
            {{0, 0}, {1, 1}, {2, 2}}, // 3
            {{0, 0}, {0, 2}, {2, 0}, {2, 2}}, // 4
    ...
    private static BufferedImage random() {
        return generate(
                ...
                fillColors[random.nextInt(fillColors.length)],
                patternColors[random.nextInt(patternColors.length)],
                random.nextInt(6)
        );
    }

    private static void randomSave(String name) throws IOException {
        BufferedImage image = random();
